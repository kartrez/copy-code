...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "openGlobalKeybindings":
			vscode.commands.executeCommand("workbench.action.openGlobalKeybindings", message.text ?? "copy-coder.")
			break
		...
		case "ghostServiceSettings":
			...
			await provider.postStateToWebview()
			vscode.commands.executeCommand("copy-coder.autocomplete.reload")
			break
		...
		case "saveApiConfiguration":
			if (message.text && message.apiConfiguration) {
				try {
					...
					await updateGlobalState("listApiConfigMeta", listApiConfig)
					vscode.commands.executeCommand("copy-coder.autocomplete.reload") // kilocode_change: Reload autocomplete model when API provider settings change
				} catch (error) {
					provider.log(
		...
		case "fetchProfileDataRequest":
			try {
				...
				if (!profile.hasSubscription) {
					...
					await provider.contextProxy.setValue("apiModelId", "openai-free")
				}
				await provider.postStateToWebview()
				provider.postMessageToWebview({
					type: "profileDataResponse",
