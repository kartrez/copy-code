...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "ghostServiceSettings":
			...
			await provider.postStateToWebview()
			vscode.commands.executeCommand("copy-coder.ghost.reload")
			break
		...
		case "saveApiConfiguration":
			if (message.text && message.apiConfiguration) {
				try {
					...
					await updateGlobalState("listApiConfigMeta", listApiConfig)
					vscode.commands.executeCommand("copy-coder.ghost.reload") // kilocode_change: Reload ghost model when API provider settings change
				} catch (error) {
					provider.log(
