...
export class AgentManagerProvider implements vscode.Disposable {
	public static readonly viewType = "copy-coder.AgentManagerPanel"
	...
	public async openPanel(): Promise<void> {
		...
		this.panel.iconPath = {
			light: vscode.Uri.joinPath(this.context.extensionUri, "assets", "icons", "kilo-white.svg"),
			dark: vscode.Uri.joinPath(this.context.extensionUri, "assets", "icons", "logo.svg"),
		}
	...
	private showAgentError(error?: { type: "spawn_error" | "unknown"; message: string }): void {
		...
		vscode.window.showErrorMessage(errorMessage, actionLabel).then((selection) => {
			if (selection === actionLabel) {
				void vscode.env.openExternal(vscode.Uri.parse("https://gpt-chat.by/docs"))
			}
		})
	}
