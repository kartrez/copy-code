...
program
	.name("kilocode")
	.description("Copy Code Terminal User Interface - AI-powered coding assistant")
	...
	.action(async (prompt, options) => {
		...
		if (!hasConfig && !hasEnvConfig) {
			...
			console.info("Welcome to the Copy Code CLI! 🎉\n")
		...
		logs.debug("Starting Copy Code CLI", "Index", { options })
...
program
	.command("auth")
	.description("Manage authentication for the Copy Code CLI")
	.action(async () => {
		await authWizard()
	})
...
program
	.command("debug")
	.description("Run a system compatibility check for the Copy Code CLI")
