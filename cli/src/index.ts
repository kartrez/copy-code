...
program
	.name("kilocode")
	.description("Copy Coder Terminal User Interface - AI-powered coding assistant")
	...
	.action(async (prompt, options) => {
		...
		if (!hasConfig && !hasEnvConfig) {
			...
			console.info("Welcome to the Copy Coder CLI! 🎉\n")
		...
		logs.debug("Starting Copy Coder CLI", "Index", { options })
...
program
	.command("auth")
	.description("Manage authentication for the Copy Coder CLI")
	.action(async () => {
		await authWizard()
	})
...
program
	.command("debug")
	.description("Run a system compatibility check for the Copy Coder CLI")
