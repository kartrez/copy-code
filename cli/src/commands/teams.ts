...
async function listTeams(context: CommandContext): Promise<void> {
	...
	if (organizations.length < 1) {
		addMessage({
			id: Date.now().toString(),
			type: "system",
			content: `You're currently not a part of any Copy Coder teams. Go to https://gpt-chat.by/get-started/teams to get started with Copy Coder for Teams!`,
			ts: Date.now(),
		})
		return
	}
...
export const teamsCommand: Command = {
	...
	arguments: [
		...
		},
		{
			name: "team-name",
			description:
				"Team name in lowercase with dashes (e.g., 'kilo-code' for 'Copy Coder') or 'personal' (for select subcommand)",
			required: false,
