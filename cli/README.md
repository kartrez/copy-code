...
Terminal User Interface for Copy Code
...
Then, make sure you place your Copy Code API token in the CLI config:

```bash
kilocode config # this opens up your editor
```

You can find your Copy Code API token on your profile page at [app.gpt-chat.by](https://app.gpt-chat.by), and place it in the `kilocodeToken` field in the CLI config.
...
When installing Copy Code CLI you'll be greeted by some scary looking dependency deprecation warnings. We're aware of the issue and will resolve it shortly.
...
Parallel mode allows multiple Copy Code instances to work in parallel on the same directory, without conflicts. You can spawn as many Copy Code instances as you need! Once finished, changes will be available on a separate git branch.
...
Autonomous mode allows Copy Code to run in automated environments like CI/CD pipelines without requiring user interaction.
